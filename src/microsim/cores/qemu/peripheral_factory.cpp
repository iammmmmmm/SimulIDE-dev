...
bool PeripheralDevice::handle_write(uc_engine *uc, uint64_t address, int size, int64_t value, void *user_data, const uint64_t simulide_time) {
  qWarning("Unprocessed operation to base class virtual function, address:%x",static_cast<unsigned>(address));
  return true;
}

bool PeripheralDevice::handle_read(uc_engine *uc, uint64_t address, int size, int64_t *read_value, void *user_data, const uint64_t simulide_time) {
  qWarning("Unprocessed operation to base class virtual function, address:%x",static_cast<unsigned>(address));
  if (read_value) {
    *read_value = 0;
  }
